...
export class ProductsTypeormRepository implements ProductsRepository {
  ...
  async findAllByIds(productIds: ProductId[]): Promise<ProductModel[]> {
    const ids = productIds.map((paramIds) => paramIds.id)

    const productsFound = await this.productsRepository.find({ where: { id: In(ids) } })
  ...
  async search(props: SearchInput): Promise<SearchOutput<ProductModel>> {
    ...
    const validdSortDir = props.sort_dir  && dirOps.includes(props.sort_dir.toLowerCase()) || false

    const orderByField = validSort ? props.sort : 'created_at'
    const orderByDir = validdSortDir ? props.sort_dir : 'desc'
